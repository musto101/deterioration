...
# the following for loop runs the nested cross-validation and stores the resulting statistic, before binding it to mcPerf.
for (i in 1:mcRep) {
...
names(mcPerf) <- 'calibration' # the resulting dataframe should have one column and one row for each iteration up to mcRep, with metrics for each iteration.
