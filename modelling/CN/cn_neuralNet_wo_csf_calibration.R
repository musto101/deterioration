...
learner = lrn("surv.deephit",
              ...
              optimizer = 'adam', early_stopping = F) # survival neural network learner initialised with hyper param search space embedded and optimizer defined as stochastic gradient descent.
...
for (i in 1:mcRep) {
  ...
  V <- rr$aggregate(msr("surv.calib_alpha")) # saves the final Calibration for the whole dataset, which results from the nested CV process.
