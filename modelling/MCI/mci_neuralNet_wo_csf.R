...
grid <- list(num_nodes = c(32, 64, 96, 128),
             activation = c('relu'),
             epochs        = c(1000, 1200, 1500, 2000),
             batch_size = c(32, 48,64), learning_rate = 0.001, alpha = seq(0.01, 0.2, 0.01),
...
for (i in 1:mcRep) {
  ...
  V <- rr$aggregate() # prints the final C-index for the whole dataset, which results from the nested CV process.
...
write.csv(mcPerf, 'data/mci_snn.csv')
