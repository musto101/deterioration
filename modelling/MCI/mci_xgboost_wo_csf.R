...
# the following for loop runs the nested cross-validation and stores the resulting C_index, before binding it to mcPerf.
for (i in 1:mcRep) {
  ...
  V = as.vector(rr$aggregate(msr("surv.cindex")))
...
write.csv(mcPerf, 'data/mci_xgBoost.csv')
