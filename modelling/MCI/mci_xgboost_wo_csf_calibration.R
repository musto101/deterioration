...
mcRep <- 100 # defines the number of reps for the monte-carlo process. Currently set at 1.
...
grid <- list(alpha = c(0, 0.01, 0.1, 0.5),
             lambda = c(0, 0.01, 0.1, 0.5),
             eta = seq(0.01, 0.02, 0.001),
             min_child_weight = c(0.001, 0.0001, 0.01),
             max_depth = c(1:10),
             nrounds = 1500)

learner = lrn("surv.xgboost",
              alpha  = to_tune(grid$alpha),
              #lambda = to_tune(grid$lambda),
...
at = auto_tuner(method = tnr("grid_search"), # defines the tuning for the inner loop incl type of hyper param tuning
                ...
                measure = msr("surv.calib_beta"), # defines the metric used to evaluate the performance of the model (C_index) for each fold
                terminator = trm("run_time", secs = 100)) # a largely redundant failsafe denoting that the processing will stop if it takes longer than 60 seconds.
...
for (i in 1:mcRep) {
  ...
  V = as.vector(rr$aggregate(msr("surv.calib_beta")))
...
names(mcPerf) <- 'calibration' # the resulting dataframe should have one column and one row for each iteration up to mcRep, with c_indices for each iteration.
