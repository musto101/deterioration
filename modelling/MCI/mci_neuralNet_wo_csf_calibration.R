...
mcRep <- 10 # defines the number of reps for the monte-carlo process. Currently set at 10
...
dat <- read.csv('data/mci_preprocessed_wo_csf.csv') # reads the mci without csf data into R.
...
grid <- list(num_nodes = c(16, 32),
             activation = c('relu'),
             epochs        = c(40, 50, 60),
             batch_size = c(32, 48), learning_rate = 0.001) # defines the hyperparameter search space. THIS SPACE RESULTED IN C_INDEX OF 0.7!
learner = lrn("surv.deephit",
              ...
              optimizer = 'adam') # survival neural network learner initialised with hyper param search space embedded and optimizer defined as stochastic gradient descent.
...
for (i in 1:mcRep) {
  ...
  V <- rr$aggregate(msr("surv.calib_alpha")) # saves the final C-calibration for the whole dataset, which results from the nested CV process.
